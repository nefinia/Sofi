...
if halfplot:
    ...
    for i in np.arange(n):
        ...
        for k in range(2):
            ...
            if usedat:
                ...
                mu[k][i] = np.nanmean(fmix)
                mu2[k][i] = ff[0][0]
                ss2 = '%s/%s/plots/%s/%s' % (folder, fitcat, extraname[1:], '%s%s%d.png' % (variables[i], s3[k], medians[i]))
    ...
    labels = [r'$\rm{Luminosity}$', r'$\rm{Number\,\,of\,\,Neighbours}$', r'$\rm{Redshift}$', r"$\rm{Comoving\,\,Distance}$",
...
if sbplot:
    ...
    if 1:
        prename = ''#'.reject3'#'.test2noreject'#''.test1sclip3'#'.reject-abitless'#
        folder = '../../%s/stacks/d0-20_pd16-80_z2.9-4.0_l1-2000_n8-100%s/' % (fitcat, prename)
        extraname = '_nmin8'
        ext = 'pdf'
    ...
    if random:
        ...
        SBstdcool = np.nanstd([np.nanmean(f1[zi:zf, yi:yf, xi:xf]-f2[zi:zf, yi:yf, xi:xf])*c for f1, f2 in zip(ft, frs)])
    ...
    if 0:#os.path.isfile(sbdat) and not overwrite:
        ...
        if simplestack:
            ...
            astroim(name.replace('.fits', '.IM.fits'), smooth=smooth, saveim=makeim, show=False, cbfrac=.08, pad=.006,
                    dfig=(8, 10), contours=contours, scb_label=label, title=title, vmin=vmi, vmax=vma, std=stdd, pair=False,
        ...
        if whitestack:
            ...
            vma = .5
            xmin, xmax, ymin, ymax = [-xl/2, xl/2, -yl/2, yl/2]
            astroim(name.replace('_white.fits', '.fits'), smooth=smooth, saveim=makeim, show=False, cbfrac=.08, pad=.006,
                    dfig=(8, 10), contours=contours, scb_label=label, title=title, vmin=vmi, vmax=vma, std=stdd, pair=False,
                    highq=highq, sb=sb, y0=0, x0=0, gray=True, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, arrow=arrow)
...
#        ywidths = [40, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 7]
        ...
        tpos = [[[-xwidths[0]*.95, -ywidths[0]*.9], [-xwidths[0]*.95, ywidths[0]*.95], [xwidths[0]*.6, -ywidths[0]*.9]],
        ...
        regions = [None, [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3],
                [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3], [5, 14, -3, 3], None]#[5, 14, -3, 3]]
    ...
    if 0:
        ...
        zw0 = 2
        offset = 0
        for f, a, xmin, xmax, ymin, ymax, n, ii, ip in zip(files, angles, xmins, xmaxs, ymins, ymaxs, names, ids, idpos):
            fit = getdata(f)
    ...
    if 0:
        ...
        fitcats = ['HDFS', 'HDFS', 'HDFS', 'HDFS', 'HDFS', 'UDF', 'UDF', 'UDF', 'UDF', 'UDF', 'HDFS', 'UDF', 'UDF', 'UDF']
        nims = len(ids1)
        ...
        ii1 = data['id1']
        ii2 = data['id2']
        angs = [angles[(ii1 == i1) & (ii2==i2)][0] for i1, i2 in zip(ids1, ids2)]#[0.291037, 1.570796, -0.245091, 1.325986, -2.466991, 2.356512]
        ...
        if 1:
            for i in (range(nims)):
                astroim('../../%s/pairs/not-normalized/masked/cubical_pair%d-%d.IM.fits' % (fitcats[i], ids1[i], ids2[i])
                ...
                astroim('../../%s/pairs/not-normalized/masked/cubical_pair%d-%d.IM.fits' % (fitcats[i], ids2[i], ids1[i])
                        , smooth=True, saveim=True, show=show,
                        cbfrac=.08, pad=.006, dfig=(8, 10), contours=True, highq=True,
...
if hm:
    ...
    def calc(z):
        ...
        J = data['col%d' % (p0 + 2)] * (1 - m) + data['col%d' % (p1 + 2)] * m  # +2 since J columns start from #2
        suma = 0
...
if circhist:
    ...
    if os.path.isdir(figfold): print 'Folder %s already exists.' % figfold
    else: glob.os.makedirs(figfold)
    catfold = outf+'/cats/angle_hist/'
    if os.path.isdir(catfold): print 'Folder %s already exists.' % catfold
    else: glob.os.makedirs(catfold)
    ...
    for nj in range(Nlae):
        j = nwsort[nj]
        good = (id1 == ids[j]) & (cds>.5) & (cds<20.) & (np.abs(x2-x1[j])>3) & (np.abs(y2-y1[j])>3) & (np.abs(y2-y1[j])>3)
        ang = angles[good]
        cd = cds[good]
        if warea: wd = weight(x1[j], y1[j], z1[j], x2[good], y2[good], z2[good])
        w = np.zeros(N)
        for i in range(N):
            if i==0: good2 = (ang%(2*np.pi)>(theta[i]-delta)%(2*np.pi)) | (ang%(2*np.pi)<=(theta[i]+width+delta)%(2*np.pi))
            elif i==N-1: good2 = (ang%(2*np.pi)>(theta[i]-delta)%(2*np.pi)) | (ang%(2*np.pi)<=(theta[i]+width+delta)%(2*np.pi))
            else: good2 = (ang>theta[i]-delta) & (ang<=theta[i]+width+delta)
            if warea: ww = 1/(cd[good2]*wd[good2])
            else: ww = 1/cd[good2]
