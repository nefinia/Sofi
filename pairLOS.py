...
#IDEA: stack spectra a la quasar spectra -> find CGM absorption
#LAEs probing LAEs
...
outtype = rdarg(argv, 'outtype', str, 'txt')
...
xw = rdarg(argv, 'xw', int, 10)
xmore = rdarg(argv, 'xmore', int, 0)#100 for eagle
yw = rdarg(argv, 'yw', int, 10)
...
output = '%s/%s/cats/pairLOS.%s' % (folder, fitcat, outtype)
out = open(output, 'w')
				...
				if z2 > z1:
					...
					if not os.path.isfile(fitsname) or imov:
						if docalc:
							...
							output = '%s/%s/cats/%s/LOS%d-%d.%s' % (folder, fitcat, extra1, i1, i2, outtype)
							px = px[cool]
							py = py[cool]
							pz = pz[cool]
							sb = cube[cool]
