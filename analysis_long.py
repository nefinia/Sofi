	...
	for snap in snaps:
		...
		if init_evol:
			...
			finit = 'SO.snap_%s_%d_%d_%d_%s_%s_%.2E.ts0000' % \
					(sname, minres, maxres, npref, _sgamma, _sheat, ssthr[s])
		...
		if col_dens:
			...
			if os.path.isfile(finit):
				...
				_fl = '%sSO.snap_%s_%d_%d_%d_%s_%s_%.2E.ts0000_var_%s_proj_%d_lmax_%d_l_' % \
								 (_s, sname, minres, maxres, npref, _sgamma, _sheat, ssthr[s], type, 1, lmax)
				isfiles = True
				for i in range(nl):
		...
		for coord, proj in zip(['x'], [1]):#zip(coordnames, [1, 2, 3]):
			...
			if do_layers:
				...
				outname = '%s/snap%d_%s_%s_%d_%d_%d_%d_%.2E.%s.fits' % (cubefolder, snap, coord, model, minres, maxres, npref, nl, ssthr[s], type)
				if not os.path.isfile(outname) or overwrite:
					cubes = []
					for i in range(nl):
							flayer = '%sSO.snap_%s_%d_%d_%d_%s_%s_%.2E.ts0000_var_%s_proj_%d_lmax_%d_l_%d_%d.fits' % \
									 (_s, sname, minres, maxres, npref, _sgamma, _sheat, ssthr[s], type, proj, lmax, i+1, nl)
			...
			if do_lls:
				...
				fnhiname = '%s/snap%d_%s_%s_%d_%d_%d_%d_%.2E.NHI.fits' % (cubefolder, snap, coord, model, minres, maxres, npref, nl, ssthr[s])
			...
			if do_dndx:
				...
				cname = 'snap%d_%s_%s_%d_%d_%d_%d_%.2E.%s' %\
						   (snap, coord, model, minres, maxres, npref, nl, ssthr[s], _type)
				if _type=='NHIcorr':
...
if laemask:
	...
	mnames = ['DATACUBE_UDF-MOSAIC.IM.Objects_Id', 'UDF10.z1300.IM.Objects_Id',
			  'DATACUBE-HDFS-1.35-PROPVAR.IM.Objects_Id']
	gnames = ['mosaic.galmask_%darcsec' % rad, 'UDF.galmask_%darcsec' % rad,
			  'HDFS.galmask_%darcsec' % rad]
	for i in [1]:
		ft = fitcat[i]
		name = names[i]
