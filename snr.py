...
if do_eagle:
	...
	do_emask = mask
	cat = getdata('%s/EAGLE/cats/gals_snap11.fits' % folder, 1)
	m = cat['DM_mass']
	s = '11'
	H0 = 69.3
	Ol = 0.714
...
if 1:
			...
			if os.path.isfile(ffits):
				if do_rand:
					for nr in range(nrand):
						if 1:#try:
							...
							gr = getdata(ffits.replace('.fits', '.rand.galmask%d.fits' %nr))
							zr, yr, xr = fr.shape
							_fr = fr[:, yr/2-wmax: yr/2+wmax+1, xr/2-wmax: xr/2+wmax+1]
							frand['%d' % nr].append(_fr)
							_gr = fr[:, yr/2-wmax: yr/2+wmax+1, xr/2-wmax: xr/2+wmax+1]
				...
				if 1:#try:
					...
					if do_mask:
						...
						mall.append(_mit)
						git = getdata(ffits.replace('.fits', '.galmask.fits'))
	...
	stackname = '%s/all/simplestacks/stack%s%s%s.fits' % (folder, scat, smask, extraname)
	hdu = PrimaryHDU()
	...
	if do_rand:
		for nr in range(nrand):
			...
			randstackname = '../../all/simplestacks/randstack%s%s%s.%d.fits' % (scat, smask, extraname, nr)
...
if 1:
	...
	#fin = fall[:, :, inside_muse]
	#for zz in zrange:
	#	fcool = fin[:, zlm/2+zz-_zw: zlm/2+zz+_zw+1, :, :]
	#	_f.append(np.nanmean(np.nanmean(np.nansum(fcool, 1), 1)))
	_f = np.nanmean(fmuse[:, inside_muse], 1)
	x = zrange*68.5
	sb = np.array(_f-fr)*flux2sb
	sbstd = sb-sb + _std*flux2sb
