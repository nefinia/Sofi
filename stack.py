...
folderim = rdarg(argv, 'folderim', str, '/net/astrogate/export/astrodata/gallegos/')  # '/scratch/gallegos/MUSE/'
...
if fitcat == 'all':
    ...
    if fitcat == 'mosaic10': cat2 = '%s/%s/cats/lae_pairs.fits' % (foldercat, 'UDF')
    else: cat2 = '%s/%s/cats/lae_pairs.fits' % (foldercat, fitcat)
    ...
    else: laedata = getdata('%s/%s/cats/laes.fits' % (foldercat, fitcat), 1)
    idlae = laedata['ID']
    if fitcat == 'EAGLE': flae = idlae-idlae+1
    else:
        fhst = laedata['HST_F606W']
        flae = laedata['LYALPHA_LUM']
        fhst1 = []
    ...
    if fitcat == 'udf10-mosaic': fitcats = ['UDF'] * len(data)
    else: fitcats = [fitcat] * len(data)
...
if len(close) > 1:
        ...
        if unique3:
            ...
            if random:
                for j in range(nrand):
                    ...
                    else: print '',#"Random pair %s does not exist" % pair
    nstack = len(lst)
    ...
    if random:
        ...
        for i in range(nrand):
            ...
            if len(random_lst[i]) > 0:
                ...
                frs.append(rlstname.replace('.lst', '.fits'))#if len(random_lst[i]) == nstack: frs.append(rlstname.replace('.lst', '.fits'))
                ...
                if unique and 0:
                    ...
                    if doanalysis:
                        fu, nu = stack(ulst, ustackname, imtype, sclip, zw0, makeim, title, vmin * 2, vmax * 2,
                                       ...
                                       var=False, corrmean=meancorr, randflipy=True, overwrite=overwrite, stds=sf,
                                       vb=vb, offset=offset, z1=z1, z2=z2, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, unique=unique4)
    ...
    if tests:
        ...
        fts = stack(fts, '%s/stack.t.fits' % (foldername), imtype, sclip, zw0, makeim, title, vmin, vmax,
                    ...
                    offset=offset, z1=z1, z2=z2, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, scliptype=scliptype, unique=unique4)
    ...
    if tests:
        ...
        np.savetxt('%s/stack_pairs.dat' % foldername, np.matrix(pairs_data).T,
                   header="Field id1 id2 redshift comoving_distance theta n_neigbors", fmt='%d %d %d %1.3f %1.3f %1.3f %d')
