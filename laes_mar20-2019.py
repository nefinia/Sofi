...
if fitcat == 'EAGLE':
	csub = False
	smooth = False
	cubename = 'snap%s_%s%s.%s.fits' % (snap, coord, smask, type)
	nconf = 1
...
for r, d, l, x, y, z, f, i in \
	...
	if fitcat == 'EAGLE':
		name = "%s/%s/gals/snap%d_%s/%d%s%s.%s.fits" % (folder, fitcat, snap, coord, i, extraname, smask, type)

	else:
		name = "%s/%s/LAEs/%d%s%s.fits" % (folder, fitcat, i, extraname, ('.%s' % line) * (line != 'lya'))
	...
	if os.path.isfile(name):
		...
		ax2 = head['NAXIS2']
		ax3 = head['NAXIS3']
		if ax1 != xw+1: os.system('rm %s' % name)
		if ax2 != yw+1: os.system('rm %s' % name)
		if ax3 != zw+1: os.system('rm %s' % name)
	...
	if overwrite or not os.path.isfile(name):
		...
		if fitcat == 'EAGLE':
			if galpos:
				xts = (xmin + xs[cool] - x) % xw
				yts = (ymin + ys[cool] - y) % yw
				zts = (zmin + zs[cool] - z) % zw
				for ii in range(len(cool)):
					flux[zts[ii], yts[ii], xts[ii]] = ids[cool][ii]
			else:
				xt = (xr + xmin + x) % xlim
				yt = (yr + ymin + y) % ylim
				zt = (zr + zmin + z) % zlim
			...
			for i in xr:
				...
				for j in yr:
					for k in zr:
						...
						cond = (xc >= _xmin) and (xc <= _xmax) and (yc >= _ymin) and (yc <= _ymax) and (zc >= _zmin) and (
								zc <= _zmax)
...
if simplestack:
	...
	zw0 = 1
	if fitcat == 'EAGLE':
		stack(lst, folder + "%s/gals/stack.fits" % fitcat, zw0=zw0)
	else:
		stack(lst, folder + "%s/gals/stack.fits" % fitcat, zw0=zw0)
