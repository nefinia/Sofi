	...
	for d in ds:
		...
		for snap in snaps:
			...
			for coord in coordnames:#['x']:#
				...
				sname = folder+'snap%d_%s_d0-%d_pd16-2000_nw0.000-1.000%s%s/stack.fits' % (snap, coord, d, stype, smask)
				if not os.path.isfile(sname) or overwrite: os.system('python stackEAGLE.py -snap %d '
						'-coord %s -dmax %d -overwrite True -imtype %s -mask %s' % (snap, coord, d, imtype, mask))
				stacks.append(getdata(sname))
			stack = np.nanmean(stacks, 0)
			zl, yl, xl = stack.shape
			hdu.data = stack
			fs = folder+'snap%d_d0-%d_pd16-2000_nw0-1000%s%s/' % (snap, d, stype, smask)
