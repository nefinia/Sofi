...
if fitcat == 'UDF':
	cubename = 'DATACUBEFINALuser_20141021T055145_72f74684%s.fits' % '.csub'*csub  # last version --> not nowwwww Aug2018
...
for r, d, l, x, y, z, f, i in \
	...
	if fitcat == 'EAGLE':
		...
		name = "%s/%s/LAEs/lae%d%s.fits" % (folder, fitcat, i, extraname)
	lst.append(name)
	...
	if overwrite or not os.path.isfile(name):
		...
		if fitcat == 'EAGLE':
			...
			_xmin = max(1 + cut, x - xw / 2)
			_xmax = min(xlim - cut, x + xw / 2)
			_ymin = max(1 + cut, y - yw / 2)
			...
			for i in xr:
				...
				for j in yr:
					for k in zr:
						...
						if cond: flux[k, j, i] = data_cube[zc, yc, xc]

		hdu.data = flux
		hdu.writeto(name, clobber=True)
		if fitcat == 'EAGLE': hdu.data = flux[zw / 2, :, :]
		else: hdu.data = np.nansum(flux, 0)
