...
def cic(value, x, nx, y=None, ny=1, weighting=True, wraparound=False):
    ...
    def update_field_vals(field, counts, value, ww=weighting, a=None, b=None, debug=False):
    ...
    update_field_vals(field, counts, value, weighting, x1, y1)
    update_field_vals(field, counts, value, weighting, x2, y1)
    update_field_vals(field, counts, value, weighting, x1, y2)
    update_field_vals(field, counts, value, weighting, x2, y2)
...
def astroim(im, smooth=True, xmin=None, xmax=None, ymin=None, ymax=None,
            ...
            scb_label='', saveim=False, clabelpad=None, ylabelpad=None,
            sbfix=False, units='theta', imout=None, show=True, title='', nsigma=5,
            x0=None, y0=None):
    import matplotlib.pyplot as plt
    from matplotlib import ticker
    from matplotlib.colors import LinearSegmentedColormap
    cmax = 4.0
    cmap = LinearSegmentedColormap.from_list('mycmap', [(0 / cmax, 'black'),
                                                        (1 / cmax, 'blue'),
                                                        (2 / cmax, 'red'),
                                                        (3 / cmax, 'yellow'),
                                                        (4 / cmax, 'white')]
...
def pdfim(lst, smooth=True, xmin=0, xmax=41, ymin=0, ymax=41,
          ...
          sbfix=False, units='theta', imout='test.pdf', title='', nsigma=5):
    import matplotlib.pyplot as plt
            ...
            for j in jj:
                for k in kk:
                    ...
                    if i < n:
                        ...
                        if std is None:
                            st = np.nanstd(img[img < 4 * np.nanstd(img)])
                            #print 'std with sigma clipping', std
                            if vmin is None: vmin = -5 * st
                            if vmax is None: vmax = 5 * st
                        #print img.shape, 'aaaaaaaaaaa'
...
def stack(lst, out, imtype='mean', sclip=5, zw0=1, jpeg=True, title='', vmin=None, vmax=None, npix=True, var=True,
          output=True, smooth=True):
    from pyfits import getdata, PrimaryHDU

    def cubim(data, name, zw0, label, vmi=None, vma=None, std=None, contours=True, smooth=True):
        zl, xl, yl = data.shape
        ...
        if jpeg:
            astroim(name.replace('.fits', '.IM.fits'), smooth=smooth, saveim=True, show=False, cbfrac=.08, pad=.006,
                    dfig=(8, 10), contours=contours, scb_label=label, title=title, vmin=vmi, vmax=vma, std=std)
    ...
    if n<1000:
        ...
        if npix:
            ...
            cubim(nf, out.replace('.fits', '.NPIX.fits'), zw0, label='Number of voxels', contours=False, smooth=False, vmi=0)#, vmi=np.amin(nf), vma=np.amax(nf))
            _output.append(nf)
        if var:
            fvar = np.nanvar(fits, axis=0)
            cubim(fvar, out.replace('.fits', '.VAR.fits'), zw0, label='Variance', vmi=0, vma=30)# vmi=np.amin(var), vma=np.amax(var))
            _output.append(fvar)
        ...
        for l in sample:
            ...
            del ff
        fits = np.array(fits)
        sigma = np.nanstd(fits[fits > -100])
        print 'Stack sigma (from a sample of 100 files):', sigma
        f = getdata(lst[0])
        f[f > sclip * sigma] = np.nan
        nf = np.isfinite(f)
        for l in lst[1:]:
            print "Open", l
            temp = getdata(l)
            temp[temp > sclip * sigma] = np.nan
            f = np.nansum((temp, f), axis=0)
            nf = np.nansum((np.isfinite(temp), nf), axis=0)
            del temp
        if imtype == 'mean':
            f = f/nf
        _output.append(f)
    ...
    cubim(f, out, zw0, 'Flux [1e-20 cgs]', vmin, vmax)
...
def analysis(f, nf, stdbin, stdbins, binsize, xl, yl, zl, yw0, zw0, frs=None):
    ...
    for i in stdbins:
        ...
        stack_stds.append(np.nanstd(np.concatenate((f[zl / 2 - zw0: zl / 2 + zw0 + 1, yl / 2 + yw0 + 2:, i:i + stdbin],
